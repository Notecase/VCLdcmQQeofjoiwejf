...
export function createResearchTools(
  ...
  tools.push(tool(
    ...
    },
    {
      ...
      schema: z.object({
        ...
        })).max(7).describe('List of todo items (max 7)'),
      }),
    },
  ))
  ...
  tools.push(tool(
    async ({ todoId, status }) => {
      ...
      if (!todo) {
        ...
        const match = ctx.todos.find(t =>
          t.content.toLowerCase().includes(todoId.toLowerCase())
          || t.id === todoId,
        )
  ...
  tools.push(tool(
    ...
    {
      name: 'think',
      description: 'Record a strategic thought or plan your next steps. Use this to reason about complex decisions before acting.',
      schema: z.object({
  ...
  tools.push(tool(
    ...
    },
    {
      ...
      description: 'Pause execution and request user approval before proceeding. Use for critical decisions.',
      schema: z.object({
        ...
        })).optional().describe('Custom approval options'),
      }),
    },
  ))
  ...
  tools.push(tool(
    ...
    {
      name: 'read_note',
      description: "Search and read the user's Inkdown notes by title. Returns matching notes with their content.",
