...
export function registerContextTools(server: McpServer, db: DbClient): void {
  ...
  server.tool(
    ...
    'Read recent context entries, optionally filtered by type.',
    {
      types: z.array(z.enum([
        'active_plan', 'research_done', 'course_saved',
        'note_created', 'note_edited', 'goal_set', 'soul_updated',
      ])).optional().describe('Filter by context type(s)'),
    ...
    async ({ types, limit }) => {
      try {
        ...
        const rows = entries.map((e) => [e.type, e.agent, e.summary.slice(0, 60), relativeTime(e.created_at)])
  ...
  server.tool('context_entries', 'List all recent context entries (unfiltered, last 20).', {}, async () => {
    try {
  ...
  server.tool('soul_read', 'Read user\'s soul (goals, preferences, learning style).', {}, async () => {
    try {
