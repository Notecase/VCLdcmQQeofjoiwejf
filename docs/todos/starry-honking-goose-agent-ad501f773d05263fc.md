...
- **Key Insight:** Quarto's power is that code, prose, and output live in one document. The _gap_ is that Quarto is still a "render and output" tool -- it doesn't manage your broader knowledge base, organize notes across projects, or connect research themes.
...
- **Strategic Insight:** Prism is OpenAI's bet that "2026 will be for AI and science what 2025 was for AI and software engineering." It acquired Crixet (cloud LaTeX platform) and evolved it into Prism. This validates the market -- but Prism is focused on _paper writing_, not the broader research lifecycle.
...
The MCP ecosystem validates that AI coding assistants (Claude Code, Cursor, VS Code + Copilot) are becoming the _hub_ for research workflows, with MCP servers as spokes connecting to knowledge platforms. Inkdown's opportunity: instead of being one more spoke, be a _unified hub_ that natively integrates the capabilities that currently require 5-6 separate MCP servers (notes + references + calendar + flashcards + report generation).
