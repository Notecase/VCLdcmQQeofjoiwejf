...
A chat agent waits for you. An always-on agent **works while you're away**. The fundamental difference is *temporal* — it operates on schedules that don't align with user sessions.
...
interface AgentTask {
  id: string
  userId: string
  type: 'spaced-review' | 'note-enrich' | 'gap-detect' | 'daily-digest' | 'weekly-report' | 'study-schedule'
