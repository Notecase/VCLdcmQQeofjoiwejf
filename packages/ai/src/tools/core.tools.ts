...
export const ReadBlockSchema = z.object({
  ...
  blockIndex: z
    ...
    .min(0)
    .optional()
    .describe('Index of the block to read (0-based). If not provided, reads all blocks.'),
})
...
export const ReadNoteSchema = z.object({
  ...
  includeMetadata: z
    ...
    .optional()
    .default(true)
    .describe('Include note metadata (title, created_at, etc.)'),
})
...
export const EditBlockSchema = z.object({
  ...
  blockIndex: z
    ...
    .min(0)
    .optional()
    .describe('Index of the block to edit. If not provided, edits entire content.'),
  ...
  try {
    ...
    if (input.blockIndex !== undefined) {
      ...
      if (input.blockIndex >= blocks.length) {
        return {
          success: false,
          error: `Block index ${input.blockIndex} out of range (${blocks.length} blocks)`,
        }
      }
      return {
        success: true,
