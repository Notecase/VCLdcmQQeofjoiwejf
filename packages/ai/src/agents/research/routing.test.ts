...
describe('classifyResearchRequest', () => {
  ...
  it('routes plain roadmap-style prompts to chat mode', () => {
    expect(classifyResearchRequest('make me a 4 month roadmap for learning backend engineering')).toBe('chat')
  ...
  it('routes detailed long-form roadmap prompts to markdown mode', () => {
    expect(classifyResearchRequest('create a detailed 3-month roadmap to learn RL')).toBe('markdown')
  ...
  it('routes deep research requests to research mode', () => {
    expect(classifyResearchRequest('do deep research on battery recycling and cite sources')).toBe('research')
    expect(classifyResearchRequest('research and compare VAE vs DAG with web sources')).toBe('research')
  })
  ...
  it('enables research file writes only for explicit file requests in research mode', () => {
    ...
    expect(shouldEnableResearchFiles('do deep research on climate policy and save a markdown report')).toBe(true)
    expect(shouldEnableResearchFiles('make a 4 month roadmap to learn deep learning')).toBe(false)
    expect(shouldEnableResearchFiles('create a detailed 3 months roadmap to learn deep learning')).toBe(false)
  })

  it('disables research files when chat-only is explicitly requested', () => {
    expect(
      shouldEnableResearchFiles('do deep research and save a markdown report, but respond in chat only'),
    ).toBe(false)
  })
