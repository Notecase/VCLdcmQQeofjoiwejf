...
export async function dbGetSchema(
  ...
  ToolResult<{ name: string; columns: { name: string; type: string }[]; rowCount: number }>
> {
  try {
    const result = await getDatabase(ctx.supabase, ctx.userId, input.noteId, input.databaseId)
    if (!result) return { success: false, error: 'Database not found' }
...
export async function dbCreateChartData(
  ...
  ToolResult<{ chartData: { labels: string[]; datasets: { label: string; data: number[] }[] } }>
> {
  try {
    const result = await getDatabase(ctx.supabase, ctx.userId, input.noteId, input.databaseId)
    if (!result) return { success: false, error: 'Database not found' }
...
export const databaseTools = [
  ...
  {
    name: 'db_update_rows',
    description: 'Update rows in an embedded database matching a filter',
    schema: DbUpdateRowsSchema,
    execute: dbUpdateRows,
  },
  {
    name: 'db_delete_rows',
    description: 'Delete rows from an embedded database matching a filter',
    schema: DbDeleteRowsSchema,
    execute: dbDeleteRows,
  },
  {
    name: 'db_query_rows',
    description: 'Query rows from an embedded database with optional filtering',
    schema: DbQueryRowsSchema,
    execute: dbQueryRows,
  },
  {
    name: 'db_aggregate',
    description: 'Perform aggregation (count/sum/avg/min/max) on database rows',
    schema: DbAggregateSchema,
    execute: dbAggregate,
  },
  {
    name: 'db_group_by',
    description: 'Group database rows and compute aggregations per group',
    schema: DbGroupBySchema,
    execute: dbGroupBy,
  },
  {
    name: 'db_column_stats',
    description: 'Get statistics for a specific column',
    schema: DbColumnStatsSchema,
    execute: dbColumnStats,
  },
  {
    name: 'db_sort_rows',
    description: 'Sort database rows by a column',
    schema: DbSortRowsSchema,
    execute: dbSortRows,
  },
  {
    name: 'db_get_schema',
    description: 'Get the schema (columns and types) of an embedded database',
    schema: DbGetSchemaSchema,
    execute: dbGetSchema,
  },
  {
    name: 'db_create_chart_data',
    description: 'Generate chart data from database rows',
    schema: DbCreateChartDataSchema,
    execute: dbCreateChartData,
  },
] as const
