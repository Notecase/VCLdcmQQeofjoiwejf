/**
 * Research Agent Stream Normalizer
 *
 * Converts raw LangGraph/deepagents streaming events into
 * normalized ResearchStreamEvent objects for the frontend.
 * Follows the same deduplication pattern as SecretaryStreamNormalizer.
 */

import type { ResearchStreamEvent } from '@inkdown/shared/types'
...
export class ResearchStreamNormalizer {
  ...
  normalizeText(nodeKey: string, message: StreamMessage): ResearchStreamEvent[] {
  ...
  normalizeToolResult(nodeKey: string, message: StreamMessage): ResearchStreamEvent[] {
