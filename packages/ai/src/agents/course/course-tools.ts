 ...
 *
 * AI SDK v6 tools split into 4 factories:
...
function createQueryRagTool(ctx: CourseToolContext) {
  return tool({
    description:
    ...
    execute: async ({ query, topK }) => {
      ...
      return await queryRAG(ctx.ragIndex.value, query, topK ?? 3)
    },
  })
}
...
export function createOrchestratorTools(ctx: CourseToolContext): Record<string, any> {
  let researchStarted = false

  // ---------- Deep Research ----------

  const run_deep_research = tool({
    description:
    ...
    execute: async ({ topic, focusAreas }) => {
  ...
  const index_research = tool({
    ...
    execute: async () => {
      ...
      const ragIndex = await indexResearchReport(ctx.researchReport.value)
  ...
  const query_rag = createQueryRagTool(ctx)

  // ---------- Generate Outline ----------

  const generate_outline = tool({
    ...
    execute: async ({ topic, difficulty, focusAreas, researchContext, feedbackSection }) => {
      ...
      const { model, entry } = resolveModel('course')
  ...
  const request_outline_approval = tool({
    description:
    ...
    execute: async ({ outlineJson, thinkingText }) => {
      if (ctx.approvedOutline.value) {
  ...
  const assemble_course = tool({
    ...
    execute: async ({ researchReport, thinkingTrace }) => {
...
export function createLessonWriterTools(ctx: CourseToolContext): Record<string, any> {
  let lessonsStarted = false

  const batch_generate_lessons = tool({
    description:
  ...
  return {
    batch_generate_lessons,
    query_rag: createQueryRagTool(ctx),
  }
}
...
export function createQuizWriterTools(ctx: CourseToolContext): Record<string, any> {
  let quizzesStarted = false

  const batch_generate_quizzes = tool({
    description:
...
export function createSlidesWriterTools(ctx: CourseToolContext): Record<string, any> {
  let slidesStarted = false

  const batch_generate_slides = tool({
    description:
