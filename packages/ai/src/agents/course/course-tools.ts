...
function createQueryRagTool(ctx: CourseToolContext): StructuredToolInterface {
  return tool(
    ...
    {
      name: 'query_rag',
      description: 'Query the indexed research report for context relevant to a specific topic or lesson.',
      schema: z.object({
...
export function createOrchestratorTools(ctx: CourseToolContext): StructuredToolInterface[] {
  ...
  tools.push(tool(
    async ({ topic, focusAreas }) => {
      if (ctx.researchReport.value) {
        console.log(`[CourseTools] run_deep_research SKIPPED — already complete (${ctx.researchReport.value.length} chars)`)
      ...
      const result = await runDeepResearch(topic, focusAreas, {
        ...
        onProgress: (progress) => {
          ctx.emitEvent({ type: 'progress', data: {
            ...
            overallProgress: Math.round(progress.progress * 0.50),
    ...
    },
    {
      ...
      description: 'Run deep research on the course topic using Gemini Deep Research. Returns a comprehensive research report.',
      schema: z.object({
  ...
  tools.push(tool(
    async () => {
      if (ctx.ragIndex.value) {
        console.log(`[CourseTools] index_research SKIPPED — already built (${ctx.ragIndex.value.chunks.length} chunks)`)
    ...
    {
      name: 'index_research',
      description: 'Index the research report into a RAG vector store for per-lesson context retrieval.',
      schema: z.object({}),
    },
  ))
  ...
  tools.push(tool(
    async ({ topic, difficulty, focusAreas, researchContext, feedbackSection }) => {
      if (ctx.approvedOutline.value) {
        console.log(`[CourseTools] generate_outline SKIPPED — outline already approved (${ctx.approvedOutline.value.modules.length} modules)`)
    ...
    },
    {
      ...
      schema: z.object({
        ...
        feedbackSection: z.string().optional().describe('Previous feedback to incorporate into revised outline'),
      }),
    },
  ))
  ...
  tools.push(tool(
    ...
    },
    {
      ...
      description: 'Pause and request user approval of the course outline before generating content. The agent will wait for the user to approve, edit, or reject.',
      schema: z.object({
  ...
  tools.push(tool(
    async () => {
      ...
      const modules: CourseModule[] = outline.modules.map((mod: CourseOutlineModule) => ({
        ...
        lessons: mod.lessons.map((les: CourseOutlineLesson) => ({
          ...
          order: les.order,
          status: 'available' as const,
          content: ctx.generatedLessons.get(les.title) ?? { markdown: `> **Generation failed.** This lesson could not be generated. Try regenerating the course or edit this lesson manually.\n\n## ${les.title}\n\n*Content pending*` },
        })),
      }))
    ...
    {
      name: 'match_videos',
      description: 'Match YouTube videos to lessons in the generated modules. Reads from stored state — no arguments needed.',
      schema: z.object({}),
    },
  ))
  ...
  tools.push(tool(
    ...
    },
    {
      ...
      description: 'Assemble the final Course object from stored outline and generated lessons. Reads from stored state — only optional arguments needed.',
      schema: z.object({
        researchReport: z.string().optional().describe('The research report text (reads from state if omitted)'),
  ...
  tools.push(tool(
    ...
    },
    {
      ...
      description: 'Persist the assembled course and its modules/lessons to Supabase. Reads from stored state — no arguments needed.',
      schema: z.object({}),
    },
  ))
    ...
    },
    {
      ...
      schema: z.object({
        todos: z.array(z.object({
          ...
          status: z.enum(['pending', 'in_progress', 'completed']).optional().describe('Task status (default: pending)'),
  ...
  tools.push(tool(
    async ({ todoId, status }) => {
      const todo = ctx.todos.find(t => t.id === todoId || t.text.toLowerCase().includes(todoId.toLowerCase()))
  ...
  tools.push(tool(
    ...
    {
      name: 'think',
      description: 'Record a strategic thought or plan your next steps. Use this to reason about complex decisions.',
      schema: z.object({
  ...
  return [
    tool(
        ...
        for (const lesson of allLessons) {
          ...
          while (attempts < maxAttempts) {
            ...
            try {
              ...
              let researchContext = ''
              if (ctx.ragIndex.value) {
                researchContext = await queryRAG(ctx.ragIndex.value, `${lesson.title} ${lesson.keyTopics.join(' ')}`, ctx.openaiApiKey, 3)
              ...
              const prompt = promptTemplate
                ...
                .replace(/{MODULE_CONTEXT}/g, `Module: ${lesson.moduleTitle} — ${lesson.moduleDescription}`)
                .replace(/{COURSE_CONTEXT}/g, `Course: ${outline.title} (${outline.difficulty})`)
                .replace(/{RESEARCH_CONTEXT}/g, researchContext)
                .replace(/{PREVIOUS_LESSONS}/g, generated > 0 ? `${generated} previous lessons generated.` : 'This is the first lesson.')

              const response = await model.invoke(prompt)
              lastResponseText = typeof response.content === 'string'
                ? response.content
                : response.content.map(c => ('text' in c ? c.text : '')).join('')

              const outlineLesson = { id: lesson.id, title: lesson.title, type: lesson.type, estimatedMinutes: lesson.estimatedMinutes, keyTopics: lesson.keyTopics, learningObjectives: lesson.learningObjectives, order: lesson.order }
              ...
              ctx.emitEvent({ type: 'content_progress', data: {
                moduleIndex: 0,
              ...
              console.log(`[batch_generate_lessons] DONE: "${lesson.title}" (${lesson.type}) — ${content.markdown.length} chars [${generated}/${total}]`)
              break // success
            } catch (err) {
              const msg = err instanceof Error ? err.message : String(err)
              if (attempts < maxAttempts) {
                console.warn(`[batch_generate_lessons] RETRY ${attempts}: "${lesson.title}" — ${msg}`)
                ctx.emitEvent({ type: 'thinking', data: `Retrying "${lesson.title}" (attempt ${attempts + 1}/${maxAttempts})...` })
              ...
              if (lastResponseText.length > 100) {
                ...
                if (fallback.length > 50) {
                  ...
                  ctx.completedLessons.value++
                  generated++
                  console.log(`[batch_generate_lessons] RECOVERED: "${lesson.title}" — ${fallback.length} chars (fallback extraction)`)
                  break
                }
              }
      ...
      },
      {
        ...
        description: 'Generate content for ALL lecture, video, and practice lessons in the approved outline. Reads from stored state — no arguments needed. Call exactly ONCE.',
        schema: z.object({}),
      },
    ),
  ...
  return [
    tool(
          ...
          while (attempts < maxAttempts) {
            ...
            try {
              ...
              const prompt = promptTemplate
                ...
                .replace(/{COURSE_CONTEXT}/g, `Course: ${outline.title} (${outline.difficulty})`)
                .replace(/{RESEARCH_CONTEXT}/g, researchContext)
                .replace(/{PREVIOUS_LESSONS}/g, generated > 0 ? `${generated} previous quizzes generated.` : 'This is the first quiz.')
              ...
              if (lastResponseText.length > 100) {
                ...
                if (fallback.length > 50) {
                  ...
                  ctx.completedLessons.value++
                  generated++
                  console.log(`[batch_generate_quizzes] RECOVERED: "${lesson.title}" — ${fallback.length} chars (fallback extraction)`)
                  break
                }
              }
              errors.push(`"${lesson.title}": ${msg}`)
      ...
      },
      {
        ...
        description: 'Generate content for ALL quiz lessons in the approved outline. Reads from stored state — no arguments needed. Call exactly ONCE.',
        schema: z.object({}),
      },
    ),
    createQueryRagTool(ctx),
  ...
  return [
    tool(
          ...
          while (attempts < maxAttempts) {
            ...
            try {
              ...
              const prompt = promptTemplate
                ...
                .replace(/{COURSE_CONTEXT}/g, `Course: ${outline.title} (${outline.difficulty})`)
                .replace(/{RESEARCH_CONTEXT}/g, researchContext)
                .replace(/{PREVIOUS_LESSONS}/g, generated > 0 ? `${generated} previous slide lessons generated.` : 'This is the first slide lesson.')
              ...
              console.log(`[batch_generate_slides] DONE: "${lesson.title}" — ${slides.length} slides, ${content.markdown.length} chars [${generated}/${total}]`)
              break // success
            } catch (err) {
              const msg = err instanceof Error ? err.message : String(err)
              if (attempts < maxAttempts) {
              ...
              if (lastResponseText.length > 100) {
                ...
                if (fallback.length > 50) {
                  ...
                  ctx.completedLessons.value++
                  generated++
                  console.log(`[batch_generate_slides] RECOVERED: "${lesson.title}" — ${fallback.length} chars (fallback extraction)`)
                  break
                }
              }
              errors.push(`"${lesson.title}": ${msg}`)
      ...
      },
      {
        ...
        description: 'Generate slide decks and content for ALL slide-type lessons in the approved outline. Uses gemini-3-pro-preview for higher quality. Reads from stored state — no arguments needed. Call exactly ONCE.',
        schema: z.object({}),
      },
    ),
    createQueryRagTool(ctx),
