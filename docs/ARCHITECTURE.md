...
**Removed agents** (capabilities extracted to registry): ChatAgent (RAG → `notes.search`), NoteAgent (CRUD → `notes.read`/`notes.create`), PlannerAgent routes (kept as class, exposed via `planning.decompose`).
...
| -------------------- | ----------------------- | -------------- |
| `notes.search`       | ChatAgent RAG pipeline  | Embedding only |
