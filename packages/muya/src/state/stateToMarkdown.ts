...
export default class ExportMarkdown {
      ...
      switch (state.name) {
        ...
        case 'task-list': {
          ...
          const bulletMarkerOrDelimiter =
            (meta as IOrderListState['meta']).delimiter || (meta as IBulletListState['meta']).marker
  ...
  serializeListItem(state: IListItemState | ITaskListItemState, indent: string) {
    ...
    else if (listIndentation === 'number') listIndent = ' '.repeat(this.listIndentationCount - 1)
