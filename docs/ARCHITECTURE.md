...
### Agent System — Unified Agent Mesh (3 active agents + 8 capabilities)

| Agent                  | Purpose                       | AI SDK Pattern                                           | Status   |
| ---------------------- | ----------------------------- | -------------------------------------------------------- | -------- |
| **EditorDeepAgent**    | Full editor AI with tool loop | `ToolLoopAgent` with 13+ tools + delegation tools        | Active   |
| **SecretaryAgent**     | AI planner / roadmap manager  | `ToolLoopAgent` with 15+ tools + delegation tools        | Active   |
...
**Removed agents** (capabilities extracted to registry): ChatAgent (RAG → `notes.search`, deleted), NoteAgent (CRUD → `notes.read`/`notes.create`, deleted; create action extracted to `utils/note-creator.ts`), PlannerAgent routes (kept as class, exposed via `planning.decompose`).

#### Capability Registry (`packages/ai/src/registry/`)

Any agent can discover and invoke shared capabilities via delegation tools generated by `generateDelegationTools()`. Delegation progress is visible in the UI via the existing `custom-progress` → `ActivityStream` pipeline.

| Capability           | Source                      | LLM Cost       |
| -------------------- | --------------------------- | -------------- |
| `notes.search`       | RAG pipeline (ex-ChatAgent) | Embedding only |
| `notes.read`         | Core tools                  | Zero           |
| `notes.create`       | Core tools                  | Zero           |
| `context.time`       | Date utils                  | Zero           |
| `schedule.read`      | MemoryService               | Zero           |
| `research.quick`     | ResearchAgent chat mode     | LLM call       |
| `planning.decompose` | PlannerAgent                | LLM call       |
...
4. SecretaryAgent surgical edit → AI edits only between markers
...
| `POST /api/agent/secretary`             | Yes   | EditorDeep agent (compound requests) - SSE                 |
...
User sends message (AISidebar input)
  ...
  -> Routes to appropriate handler (tools, delegation, etc.)
...
User requests edit (e.g., "shorten the introduction")
  ...
  -> streamSurgicalEdit() generates targeted changes
...
User Goal → Research → Course Draft → Daily Plan → Note Pack → Done
                 ↓            ↓              ↓            ↓
            ResearchAgent  CourseOrch.   SecretaryAgent  note-creator
                 ↓            ↓              ↓            ↓
