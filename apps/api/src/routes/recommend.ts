...
const GenerateAllSchema = z.object({
  ...
  types: z.array(z.enum(['mindmap', 'flashcards', 'concepts', 'exercises', 'resources'])).optional(),
})
...
const GenerateSlidesSchema = z.object({
  ...
  theme: z.enum(['Technical/Engineering', 'Organic/Biological', 'History/Paper', 'Modern/Abstract']).optional(),
})
...
recommend.post(
  '/generate',
  zValidator('json', GenerateAllSchema),
  async (c) => {
...
recommend.post(
  '/mindmap',
  zValidator('json', GenerateTypeSchema),
  async (c) => {
    const auth = requireAuth(c)
    const body = c.req.valid('json')
    const openaiApiKey = process.env.OPENAI_API_KEY
...
recommend.post(
  '/slides',
  zValidator('json', GenerateSlidesSchema),
  async (c) => {
    const auth = requireAuth(c)
    const body = c.req.valid('json')
      ...
      return streamSSE(c, async (stream) => {
        try {
          const slides = await generateSlides(
            ...
            geminiApiKey,
            body.numSlides,
            async (progress: { currentSlide: number; totalSlides: number; status: string; message: string }) => {
