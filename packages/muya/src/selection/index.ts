...
class Selection {
  ...
  selectAll() {
    ...
    if (activeEle && activeEle.classList.contains('mu-content')) (activeEle as HTMLElement).blur()
  }
  ...
  private listenSelectActions() {
    ...
    const handleMouseupOrLeave = () => {
      if (this.selectInfo.selection) this.setSelection(this.selectInfo.selection)
    ...
    const handleClick = (event: Event) => {
      const { target } = event
      const imageWrapper = (target as HTMLElement)?.closest(`.${CLASS_NAMES.MU_INLINE_IMAGE}`)
      this.selectedImage = null
      if (imageWrapper) return this.handleClickInlineImage(event, imageWrapper as HTMLElement)
    }
  ...
  private handleClickInlineImage(event: Event, imageWrapper: HTMLElement) {
    ...
    const { target } = event
    const deleteContainer = (target as HTMLElement).closest('.mu-image-icon-close')
  ...
  private select(startNode: Node, startOffset: number, endNode?: Node, endOffset?: number) {
