...
class FootnoteTool extends BaseFloat {
    ...
    override listen() {
        ...
        super.listen();
        eventCenter.subscribe('muya-footnote-tool', ({ reference, identifier, footnotes }: FootnoteToolPayload) => {
            if (reference) {
    ...
    render() {
        const { _oldVNode: oldVNode, _toolContainer: toolContainer, _identifier: identifier, _footnotes: footnotes } = this;
        if (!identifier || !footnotes)
            return;
